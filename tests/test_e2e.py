...
def test_verbose_output(tmp_path: str) -> None:
    ...
    with runner.isolated_filesystem(tmp_path) as temp_dir:
        Path(temp_dir).joinpath("hello.py").write_text(
...
def test_ignore_single_files(tmp_path: str) -> None:
    ...
    with runner.isolated_filesystem(tmp_path) as temp_dir:
        ...
        for filename in filenames:
            dirpath.joinpath(filename).write_text(content)
    ...
    assert content in dirpath.joinpath(filenames[1]).read_text()
...
def test_ignore_multiple_files(tmp_path: str) -> None:
    ...
    assert content in dirpath.joinpath(filenames[1]).read_text()
    assert content in dirpath.joinpath(filenames[2]).read_text()
...
def test_transform_failure(tmp_path: str) -> None:
    runner = CliRunner()
    with runner.isolated_filesystem(tmp_path) as temp_dir:
        Path(temp_dir).joinpath("hello.py").write_text('print("hello"\n')
